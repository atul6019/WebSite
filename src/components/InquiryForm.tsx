...
export function InquiryForm({ interest = "Custom project" }: { interest?: string }) {
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className="glass rounded-3xl p-6"><div className="grid gap-4 sm:grid-cols-2">{fields.map(([name,label,type])=><label key={name} className="text-sm text-slate-300">{label}<input type={type} {...register(name)} className="mt-2 w-full rounded-2xl border-cyan-300/10 bg-slate-950/70 text-white focus:border-primary focus:ring-primary" />{errors[name] && <span className="text-xs text-red-300">Required valid value</span>}</label>)}</div><label className="mt-4 block text-sm text-slate-300">Project Requirement<textarea {...register("requirement")} rows={5} className="mt-2 w-full rounded-2xl border-cyan-300/10 bg-slate-950/70 text-white focus:border-primary focus:ring-primary" />{errors.requirement && <span className="text-xs text-red-300">Tell us at least 10 characters.</span>}</label><button disabled={isSubmitting} className="mt-5 rounded-full bg-primary px-7 py-3 font-bold text-slate-950 transition hover:bg-accent disabled:opacity-60">{isSubmitting ? "Sending..." : "Send Inquiry"}</button>{isSubmitSuccessful && <p className="mt-3 text-sm text-emerald-300">Inquiry received. Our team will contact you shortly.</p>}</form>;
}
